...
public class ConnectionManager extends Thread{
	...
	public String waitingForMessage() {
		...
		while (!complete) {
			...
			else if (!ExplorationThread.getRunning() && !FastestPathThread.getRunning() && s.contains(Constant.SETWAYPOINT)) {
				//System.out.println("!!! ENTERED SET WAY POINT !!!");
				Pattern wp_s_s = Pattern.compile(Constant.SETWAYPOINT + " \\([1-9],[1-9]\\)");  	// 1-9	 1-9
				Pattern wp_d_s = Pattern.compile(Constant.SETWAYPOINT + " \\([1-9],[1][0-8]\\)");	// 1-9	 10-18
				Pattern wp_s_d = Pattern.compile(Constant.SETWAYPOINT + " \\([1][0-4],[1-9]\\)");	// 10-14 1-9
				Pattern wp_d_d = Pattern.compile(Constant.SETWAYPOINT + " \\([1][0-4],[1][0-8]\\)");// 10-14 10-18
