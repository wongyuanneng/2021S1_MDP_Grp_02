...
public class SimulatorRobot extends Robot{
	...
	public void forward(int step) {
		...
		this.y = checkValidY(this.y + Constant.SENSORDIRECTION[this.getDirection()][1]);
		...
		if(canMove) {
			for (int i = 0; i < step * Constant.GRIDWIDTH; i++) {
				//if (x == 0 || x == 19 && y == 0 || y ==14) // pos(x,y) on arena edge
				//	break;
				//else if(x > 0 && x <19 && y > 0 && y <14) // pos(x,y) within arena
	...
	public void backward(int step) {
		...
		this.y = checkValidY(this.y + Constant.SENSORDIRECTION[(this.getDirection()+2)%4][1]);
		switch (this.getDirection()) {
			case Constant.NORTH:
				s = "Up";
				break;
		...
		toggleValid();
		for (int i = 0; i < step * Constant.GRIDWIDTH; i++) {
			if(x > 0 && x <19 || y > 0 && y <14) // pos(x,y) within arena
				t.schedule(new MoveImageTask(robotImage, s, 1), delay * (i + 1));
			else
				break;
		}
