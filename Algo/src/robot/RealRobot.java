...
public class RealRobot extends Robot{
	...
	private RealRobot(boolean simulator) {
		...
		if (simulator) {
			JFrame frame = new JFrame("Group 2 Real Run");
			...
			sr = new SimulatorRobot(frame, this.x, this.y, 1);
	...
	public void sendMDPString() {
		//if (this.numOfCount > 3) {
			...
			String msg = "M{\"map\":[{\"explored\": \"" + arr2[0] + "\",\"length\":" + arr2[1] + ",\"obstacle\":\"" + arr2[2] + "\"}]}";
			connectionSocket.sendMessage(msg) ;
			if (sr != null) {
				sr.displayMessage("Sent message: " + msg, 1);
			}
			//this.numOfCount = 0;
		//}
	...
	// Send the left align command
	public void left_align() {
		connectionSocket.sendMessage(Constant.LEFTALIGN);
		if (sr != null) {
			sr.displayMessage("Sent message: " + Constant.LEFTALIGN, 1);
		}
		acknowledge();
	}
