package com.example.mdp_android_grp2;
...
import com.example.mdp_android_grp2.ui.main.BluetoothConnectionService;
import com.example.mdp_android_grp2.ui.main.BluetoothPopUp;
import com.example.mdp_android_grp2.ui.main.CommsFragment;
import com.example.mdp_android_grp2.ui.main.GridMap;
import com.example.mdp_android_grp2.ui.main.MapInformation;
import com.example.mdp_android_grp2.ui.main.MapTabFragment;
import com.example.mdp_android_grp2.ui.main.ReconfigureFragment;
import com.example.mdp_android_grp2.ui.main.SectionsPagerAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editor.putString("IRString","Image Recognition String:"+"\n");
        editor.commit();
        ...
        printMDFStringButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                editor.commit();
                refreshMessageReceived(message, true);
        ...
        Button printIRStringButton = (Button) findViewById(R.id.printImageRecogString);
        printIRStringButton.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){
                editor = sharedPreferences.edit();
                String message = sharedPreferences.getString("IRString","Image Recognition String:"+"\n");
                refreshMessageReceived(message, true);
            }
        });
...
//            case "starting":
    ...
    public static void refreshMessageReceived(String message, boolean sf) {
        String messageColoured = "";
        if (sf){
            messageColoured = getColoredSpanned(message, "#b1bfca");
        }
        else{
            messageColoured = getColoredSpanned(message, "#6d6d6d");
        }
        CommsFragment.getMessageReceivedTextView().append("\n");
        CommsFragment.getMessageReceivedTextView().append(Html.fromHtml(messageColoured, Html.FROM_HTML_MODE_LEGACY));
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            try {
                if (message.length() > 8 && message.substring(2,7).equals("image")) {
                    ...
                    sharedPreferences();
                    message = "["+jsonArray.getString(0)+", "+jsonArray.getString(1)+", "+jsonArray.getString(2)+"]";
                    editor.putString("IRString",sharedPreferences.getString("IRString","")+" "+message);
                    editor.commit();
            ...
            if (!gridMap.getAutoUpdate() && message.contains("status")){
                Toast.makeText(MainActivity.this, "Robot Status received. Please set map to AUTO to update Robot Status", Toast.LENGTH_LONG).show();
            }
            sharedPreferences();
            String receivedText = sharedPreferences.getString("message", "") + "\n" + message;
            editor.putString("message", receivedText);
            editor.commit();
            refreshMessageReceived(message, false);
        }
    };
